...
## Session 7 — DIC2 Value Iteration 作業

**日期：** 2026-03-18  
**目標：** 將原本的演算法改為求最佳解的 **Value Iteration**，更新預設環境，並上傳至新 Repo `Charles8745/2026DRL_DIC2`。

### 需求實作
1. **地圖預設載入**：修改 `script.js`，讓網頁一載入直接預設 `n=5` 網格，起點 `(0,0)`，終點 `(4,4)`，障礙物 `(1,1), (2,2), (3,3)`。
2. **Value Iteration 演算法 (`gridworld.py`)**：
   - 移除隨機策略，改套用 Bellman Optimality Equation：$V(s) = \max_a [r + \gamma V(s')]$。
   - 計算收斂後回傳 Deterministic optimal policy $\pi(s)$。
3. **最佳路徑視覺化除錯 (BFS Tracking)**：
   - **觀察問題**：原本程式碼在面對多個相同最高分的最佳動作時，只貪婪選取「第一個」動作，導致畫出的路徑「漂亮但死板」，無法展現 RL 全域最佳解的精神。
   - **解決方案**：改寫前端 `script.js` 最佳路徑追蹤演算法，套用 **BFS（廣度優先搜尋）**，走遍所有得分相同的最佳動作，讓畫面上呈現多條等價的繞行最短路徑。
   - 新增紫色光暈的 CSS 高亮效果（Optimal Path），加強視覺回饋。

### 重新部署
- 建立新分支 `DIC2`，推送到 `https://github.com/Charles8745/2026DRL_DIC2.git` 的 `main`。
- 修改 `render.yaml` 服務名稱為 `drl-dic2`，方便串接自動部署。

---

*本 DEVLOG 由開發過程的 AI 輔助對話紀錄整理而成，記錄包含 2026-03-18 之 DIC2 操作紀錄*
